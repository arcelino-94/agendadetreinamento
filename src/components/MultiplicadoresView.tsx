    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Buscar por nome ou especialidade (ex: PIX, Cartão)..."
            value={searchQuery}
        ...
        <div className="flex items-center space-x-2">
          <select
            ...
            className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-medium text-slate-800 dark:text-slate-200"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center space-x-1.5 shrink-0"
          >
          ...
          >
              ...
              <div className="space-y-2 text-xs text-slate-600 dark:text-slate-400">
                ...
                <div className="space-y-1">
                  <div className="flex items-center space-x-1.5 text-slate-400 font-medium text-[11px]">
                    ...
                    <span>Especialidades Dominadas:</span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {m.especialidades.map((esp, i) => (
                      <span 
                        key={i} 
                        className="bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-200 px-2 py-0.5 rounded text-[10px] font-medium"
                      >
                        {esp}
                      </span>
                    ))}
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
              ...
              <div className="space-x-1">
                ...
                <button
                  onClick={() => {
                    if (confirm(`Excluir o multiplicador ${m.nome}?`)) {
                      deleteMultiplicador(m.id);
                    }
                  }}
      ...
      {/* Modal Modal Add/Edit Multiplicador */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <form 
            onSubmit={handleSave}
            className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full border border-slate-200 dark:border-slate-800 p-6 space-y-4 shadow-2xl"
          >
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-600 dark:text-slate-400 font-semibold mb-1">E-mail Corporativo:</label>
                  <input
                    type="email"
                    required
                    value={email}
                ...
                <div>
                  <label className="block text-slate-600 dark:text-slate-400 font-semibold mb-1">Telefone/Ramal:</label>
                  <input
                    type="text"
                    value={telefone}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-600 dark:text-slate-400 font-semibold mb-1">Horário Início:</label>
                  <input
                    type="time"
                    required
                    value={horarioInicio}
                ...
                <div>
                  <label className="block text-slate-600 dark:text-slate-400 font-semibold mb-1">Horário Fim:</label>
                  <input
                    type="time"
                    required
                    value={horarioFim}
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Ex: PIX, Cartão, Retenção, Novatos"
              ...
              <div>
                <label className="block text-slate-600 dark:text-slate-400 font-semibold mb-1">Status Atual:</label>
                <select
                  value={status}
              ...
              <div>
                <label className="block text-slate-600 dark:text-slate-400 font-semibold mb-1">URL da Foto Avatar (opcional):</label>
                <input
                  type="text"
                  value={foto}
